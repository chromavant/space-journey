/*==========================================================
 ...
 This program is still being worked on due to the awesome
...
var maxAsteroids = 50; 
var starCount = 150; // amnt of stars
...
// Technical Constants /////////////////////////////////////
// amnt of ship friction, higher is looser NOT IMPLEMENTED
//var shipFriction = 0.05; 
var hyperSpeed = 0.9;
var maxDmgTick = 0.03;
var DEBUG = true;
var showFPS = DEBUG;
var programStartTime = millis();
...
var isInCenteredRegion = function(
    ...
    if (curX > regX - (regW / 2) && curX < regX + (regW / 2) && 
        curY > regY - (regH / 2) && curY < regY + (regH / 2)
    ){
        return true;
    }
    return false;
};
// works for a center of entity inside a left upper corner origin region. // works for other regions?
var subjectInRegion = function(subject, region){
    if ( subject.x <= region.w + (subject.w/2) && subject.x >= region.x - (subject.w/2) &&
         subject.y <= region.h + (subject.h/2) && subject.y >= region.y - (subject.h/2) 
    ){
        return true;
    }
    return false;
    ...
    if (start !== target){
        if ((increment > 0 && start < target) || (increment < 0 && start > target)){
            result = start + increment;
             // constrain to target
            if ((result > target && increment > 0) || (result < target && increment < 0)) {
                result = target;
            }
            return result;
        }
...
var getEllipsePoint = function(d, xr, yr){
    if ( !yr ){
        yr = xr;
    }
    var a = cos(d) * xr;
...
var randomNameGen =  function (){
    var syllables  = [ 
        "do", "re", "me", "fa", "so", "la", "de", "do", "he", "rp", "de", "rp", "as", "or", "f", "fa", "ma", "an", "ti", "t", "si", "il", "al", "pa", "l", "on", "ke", "es", "lu", "ha", "ee", "sh", "is", "in", "ob", "da", "o", "en", "co", "ro", "pi", "ae", "gl", "ar", "a", "l", "p", "u", "uu", "za", "al", "di", "kh", "tr", "z", "g", "kh", "ur", "az", "ib", "zh", "bi", "po", "do", "im", "de", "so", "kl", "ep","om", "os", "lo", "er","he", "gl", "rg", "gu", "ua", "r", "on", "zi", "in", "bl", "b", "op", "za", "ap", "zi", "ip", "z", "i", "p", "a", "ea", "oa", "ow", "ot", "ge", "ek", "ne", "rd", "hi", "ip", "ca", "ka", "gl", "e", "bu", "ul", "lu", "ru", "ur"
    ];
...
var Slider = function( title, minVal, maxVal, defaultVal, x, y, w, h, txtOffsetX, txtOffsetY ){
    /* 
...
ui.sliders = {
    list: {},
    add: function( id, title, minVal, maxVal, defaultVal, x, y, w, h, txtOffsetX, txtOffsetY ){
...
var notices = {
    ...
    add: function(type, msg, duration){
        ...
        if (this.list.length === 0 || this.list[this.list.length-1].msg !== msg ){
...
ui.dialog = {
    ...
    cancelTxt: "",
    cancelAction: null,
    open: function(text, confirmAction, cancelAction,
        ...
        ui.buttons.add("dialogConfirm", confirmTxt, 
            width/2 + this.w*0.25, height/2 + this.h/2 - 17,
            100, 25);
...
//===========================================================
// Events Framework /////////////////////////////////////////
// condition, action, repeatTime (ms delay / optional)
// single time event if no repeatTime included
var Event = function (condition, action, repeatTime) {
...
var events = { 
    list : [],
    register: function(condition, action, repeatTime) {
        this.list.push( new Event(condition, action, repeatTime) );
    },
    schedule: function(time, action, repeatTime) {
    ...
    service: function () {
        for (var i = 0; i < this.list.length; ++i) {
            ...
            //println(event.repeatTime + ": " + event.lastTime);
            if (event.lastTime === undefined ||
               (sessionDuration >= event.lastTime + event.repeatTime)
            ) {
                if (event.condition()) {
                    event.action();
                    if (event.repeatTime === undefined) {
                        this.list.splice(i, 1);
                    }
                    else {
...
var stars = {
    ...
    extendFromFocus: function(x, y, lengthFactor){
        var distanceMag = lowerLimit(
                dist(sFocus.x, sFocus.y, x, y)/400, 0.3
            );
        lengthFactor *= distanceMag;
        return { //sFocus is a global
            //change in x * length
            x: x + ((x - sFocus.x) * lengthFactor), 
            //change in y * length
            y: y + ((y - sFocus.y) * lengthFactor)  
        };
    },
...
var Ship = function (){
    ...
    this.speed = 0.8;
    this.throttle = 0.8/this.p.speedMax;
    this.momentum = 0;
    // Display Properties
    this.rotation = 0;
    this.zoom = 1.0;
    ...
    this.takeDmg = function(dmg){
        ...
        var adjustedDmg = ( 
            dmg - (dmg * ship.s.hull.rating) ) * 
            dmgMultiplier;
        if (!adjustedDmg){
            adjustedDmg = minimumDmgTick;
        }
        this.setHull(this.s.hull.amnt - adjustedDmg);
        info.lastDamager = this.damager;
    };
    // not sure we need this
    this.setHull = function(newHull){ 
        this.s.hull.amnt = constrain(
            newHull, 0.0, ship.s.hull.cap);
    };
    // Navigation Methods
    ...
    this.matchPos = function(){
        ...
        var momentumLoss = 0.01 + 
            (0.01 * ( this.p.speedMax - this.speed ) );
    ...
    this.matchSpeed = function(){
        ...
        var speedMatchProgress = this.speed/this.p.speedMax;
        var baseRate = lowerLimit( 
            0.01 * speedMatchProgress, 0.001 
        );
        var rateModifier = this.p.acceleration;
        
        if (this.speed > newTarget){
            baseRate = -baseRate;
            rateModifier = this.p.braking;
        }
    ...
    this.setFuel = function(newFuel){ 
        this.s.fuel.amnt = constrain(newFuel, 0.0, ship.s.fuel.cap);
    };
    // Energy Methods
    this.setEnergy = function(newEnergy){
        this.s.energy.amnt = constrain(
            newEnergy, 0.0, ship.s.energy.cap
        );
    };
    // not sure we need this
    this.setBarrier = function(newBarrier){ 
        this.barrier = constrain(
            newBarrier, 0.0, this.p.barrierMax
        );
    };
    // Display Methods
    ...
    this.drift = function(driftDistance, driftIncrement){ 
        if (this.pos.x > sFocus.x + random(driftDistance) ||
            this.pos.x < sFocus.x - random(driftDistance) ||
            this.pos.y > sFocus.y + random(driftDistance) ||
            this.pos.y < sFocus.y - random(driftDistance)
        ){
...
// Ship.draw Method ////////////////////////////////////////
Ship.prototype.draw = {
    lightSpeedEffect: function(zoom, speed){
        // top fin speed effect (fade in, flicker)
        fill(random(200, 205), random(250, 255), random(250, 255), random(50 * (0.1 + speed)) );
        //(shape, localX, localY, x, y, w, h, scale){
        ellipse( 0, zoom*-22, zoom*14, zoom*30);
        // ship rear speed effect  (fade in, flicker)
        fill(random(250, 255), random(250, 255), random(250, 255), random(20 * (0.1 + speed)) );
        ellipse( 0, zoom*-1, zoom*90, zoom*40 );
        // wing speed effect (fade in, flicker)
        fill(random(250, 255), random(250, 255), random(250, 255), random(40 * (0.1 + speed)) );
        ellipse( 0, zoom*-1, zoom*150, zoom*20 );
    }, 
    topFin: function(zoom){
        // top fin (bright flicker)
        fill(random(135, 137), random(135, 137), random(135, 137));
        ellipse( 0, zoom*-22, zoom*8, zoom*25 );
        fill(random(155, 157), random(155, 157), random(155, 157));
        ellipse( 0, zoom*-22, zoom*4, zoom*20 );
    },
    wings: function(zoom){
        // wings (bright flicker)
        fill(random(170, 172), random(170, 172), random(170, 172));
        ellipse( 0, zoom*1, zoom*130, zoom*12 );
        fill(random(138, 140), random(138, 140), random(138, 140));
        ellipse( 0, 0, zoom*120, zoom*10 );
    },
    rear: function(zoom){
        // ship rear (bright flicker)
        fill(random(140, 150), random(140, 150), random(140, 150));
        ellipse( 0, 0, zoom*60, zoom*30 );
        fill(random(163, 173), random(163, 173), random(163, 173));
        ellipse( 0, 0, zoom*50, zoom*30 );
    },
    exhaust: function(zoom){
        // exhaust back shadow (bright flicker)
        fill(random(90, 100), random(90, 100), random(90, 100) );
        ellipse( 0, 0, zoom*20, zoom*20 );
        fill(random(113, 123), random(113, 123), random(113, 123) );
        ellipse( 0, 0, zoom*20, zoom*20 );
        // exhaust metal (bright flicker)
        fill(random(170, 180), random(170, 180), random(170, 180) );
        ellipse( 0, 0, zoom*30, zoom*30 );
        fill(random(143, 153), random(143, 153), random(143, 153) );
        ellipse( 0, 0, zoom*26, zoom*26 );
        fill(0, 0, 0, 75);
        ellipse( 0, 0, zoom*16, zoom*16 );
        fill(255, 255, 255, 50);
        ellipse( 0, 0, zoom*8, zoom*8 );
    },
    damage: function (zoom){
        ...
        var w = 252;
        var h = 94;
        fill(0, 0, 0, (ship.s.hull.cap - ship.s.hull.amnt) * 65);
        stroke(61, 61, 61, (ship.s.hull.cap - ship.s.hull.amnt) * 65);
        beginShape();
            var vertices = [
            // 87 Total  Points
               [0, 66], [15, 65], [24, 73], [40, 63], [32, 67], [55, 62], [54, 68], [59, 76], [67, 62], [66, 58], [75, 57], [73, 67], [86, 66], [76, 71], [87, 80], [92, 79], [92, 73], [104, 59], [104, 66], [101, 74], [112, 79],[110, 85], [100, 89], [123, 94], [122, 85], [131, 81], [142, 92], [143, 87], [138, 81], [126, 70], [134, 62], [134, 59], [115, 67], [112, 56], [122, 46], [125, 0], [129, 3], [124, 19], [128, 34], [133, 14], [132, 28], [125, 58], [140, 44], [136, 60], [148, 62], [143, 72], [152, 93], [169, 85], [152, 81], [159, 78], [153, 71], [163, 68], [172, 76], [179, 68], [193, 71], [200, 65], [200, 55], [212, 62], [209, 61], [235, 63], [246, 64], [252, 67], [221, 67], [213, 71], [198, 61], [179, 59], [162, 60], [168, 55], [172, 47], [149, 54], [153, 45], [161, 41], [139, 35], [115, 39], [99, 47], [95, 58], [82, 52], [93, 50], [88, 49], [92, 47], [89, 42], [108, 37], [71, 58], [43, 59], [20, 61], [24, 66], [3, 66]
            ];
            //dMsg("x" + x);
            for ( var i = 0; i < vertices.length; i++ ){
                vertex( ship.pos.x - (w*zoom/2) + 
                    (vertices[i][0]*zoom), 
                    ship.pos.y - (h*zoom/1.45) + 
                    (vertices[i][1]*zoom)
                );
            }
        endShape();
    },
    flames: function(zoom, speed){
        var speed = constrain(speed, 0, 1.1);
        noStroke();
        // engine flames (fade in)
        var white = color(255, 255, 255, 210 * speed);
        var outerFlame0 = color( random(200, 220), 94 + 
            (50 * speed), 7, 
            90 * speed );
        var outerFlame1 = color( random(200, 220), 94 + 
            (50 * speed), 7,
            240 * speed );
        var outerFlame2 = color( random(190, 210), 94 + 
            (50 * speed), 7,
            240 * speed );
        var outerFlame3 = color( random(219, 239), 74 + 
            (50 * speed), 7,
            245 * speed );
        // flame center (fade in)
        var innerFlame1 = color( random(219, 239), 200 + 
            (50 * speed), 7,
            70 + random(80 * speed) );
        var innerFlame2 = color( 200, 200, random(200, 255), 
            70 + random(80 * speed) ); //0.01 ?
        var newSpeed = speed;
        if (speed > 0.85){
            // engine flames (fade in)
            outerFlame0 = color( 20, 94 + (50 * speed), 
                random(200, 220),
                (90 * speed) );
            outerFlame1 = color( 20, 94 + (50 * speed), 
                random(200, 220),
                (240 * speed) );
            outerFlame2 = color( 20, 94 + (50 * speed), 
                random(190, 210),
                (240 * speed) );
            outerFlame3 = color( 20, 74 + (50 * speed), 
                random(219, 239),
                (245 * speed) );
            // flame center (fade in)
            innerFlame1 = color( 20, 200 + (50 * speed), 
                random(219, 239),
                70 + random(80 * speed) );
            innerFlame2 = color(random(225, 255), 
                random(225, 255), 255, 
                70 + random(80 * speed) ); //0.01 ?
            newSpeed = 0.85/speed;
            // subtle hyper speed entrance/exit effect
        ...
        //----------------------------
        outerFlame0 = lerpColor( outerFlame0, white, 0.65 *
            newSpeed * newSpeed );
        outerFlame1 = lerpColor( outerFlame1, white, 0.65 *
            newSpeed * newSpeed );
        outerFlame2 = lerpColor( outerFlame2, white, 0.65 *
            newSpeed * newSpeed );
        outerFlame3 = lerpColor( outerFlame3, white, 0.65 *
            newSpeed * newSpeed );
        innerFlame1 = lerpColor( innerFlame1, white, 0.65 *
            newSpeed * newSpeed );
        innerFlame2 = lerpColor( innerFlame2, white, 0.65 *
            newSpeed * newSpeed );
        //----------------------------
        // engine flames (fade in)
        fill(outerFlame0);
        ellipse( 0, 0, zoom*random(30, 35), 
            zoom*random(30, 35) );
        fill(outerFlame1);
        ellipse( 0, 0, zoom*random(20, 28), 
            zoom*random(20, 28) );
        fill(outerFlame2);
        ellipse( 0, 0, zoom*random(20, 25), 
            zoom*random(20, 25) );
        fill(outerFlame3);
        ellipse( 0, 0, zoom*random(15, 20), 
            zoom*random(15, 20) );
        // flame center (fade in)
        fill(innerFlame1);
        ellipse( 0, 0, zoom*random(15, 20) * speed, 
            zoom*random(15, 20) * speed);
        fill(innerFlame2);
        ellipse( 0, 0, zoom*random(10, 15) * speed, 
            zoom*random(10, 15) * speed );
...
Ship.prototype.display = function(){
    ...
    pushMatrix();
        ...
        translate(this.pos.x, this.pos.y);
        draw.lightSpeedEffect(zoom, speed); 
        draw.topFin(zoom);
        draw.wings(zoom);
        draw.rear(zoom);
        draw.exhaust(zoom);
...
Ship.prototype.refresh = function(){
    ...
    if (this.s.fuel.amnt <= 0){ // no fuel
        ...
        var fuelRatingFactor = (1.0 / this.s.fuel.rating);
        this.s.fuelTick = ( // assume max rating of 1.0
            baseFuelTick * this.p.fuelRate * 
            fuelRatingFactor * this.speed
        );
        this.setFuel(this.s.fuel.amnt - this.s.fuelTick);
        //dMsg(this.s.fuel.amnt);
        if (this.s.fuel.amnt < 0.05){ 
            // not sure if randomizing this makes problems
            // fuel stutter
            this.setThrottle(random(this.s.fuel.amnt));
        }
    }
...
//==========================================================
// Custom ships
var SuperShip = function (){ // parasitic inheritance
    var newShip = new Ship();
    newShip.p = { // properties
...
entities.dealLandingDamage = function (le){
    ...
    if (ship.landing.progress > le.landingDmgRange){
        var landingDmgMultiplier = 1 + 
            ship.landing.progress - le.landingDmgRange;
...
entities.handleCollisionTarget = function(te){
    ...
    if (te.type === "asteroid"){
        ...
        if (!ship.landing.progress){
            if (shipOverThisEntity && te.zoom >= this.inRangeZoom ){
...
entities.updateMovementVisuals = function(te){
    ...
    te.shiftByInc(sFocus.x, sFocus.y,
...
entities.updateAll = function(){
    ...
    for (var i = 0; i < pipeline.length; i++){
        te = pipeline[i]; // get object from pipeline
        // using visibleSize works until we get entities
        // that are not 1:1 zoomed
        // assume for now all regions are symmetrical
        var totalVisibleSize = te.effectSize * te.zoom;
        var entityInView = subjectInRegion( 
            {x: te.x, y: te.y, w: totalVisibleSize, h: totalVisibleSize},
            {x: 0,    y:0,     w: width,            h: height}
        );
        if (entityInView) {
            // add to new pipeline
            newPipeline.push(te);
            //println(te.type + " : " + te.x  + ", " + te.y + ", " + te.size + ", " + te.zoom);
            if (te !== ship.landing.entity){
                if (gameMode) {
                    this.handleCollisionTarget(te);
                }
                if (!ship.landing.progress ){
                    this.updateMovementVisuals(te);
                }
            }
            try {
                te.draw(); // (i)   
                ...
                println("Fatal Error: Failed to draw '" + te.type + "'\nConsole: " + e);
                noLoop();
            }
        } else if (ship.landing.entity === te){
...
var PlanetRing = function(planetRadius, planetColor){
    this.list = [];
    var maxIncrement = 0.1 + random(0.5);
    ...
    for (var scale = ringScaleMin; scale < ringScaleMax; scale += ringScaleGap ){
        ...
        this.list.push([  [newRadius*2, newRadius], particleSize, particleColor ]);
    }
};
...
Plant.prototype.getYSinPos = function(y){
    ...
    var bulge = cos(y * this.bulge); 
    var taper = (1-(y/this.height)*this.taper);
    var nextPos = new PVector(
        cos(y * this.yUnitCompress) * this.xUnitWidth * taper *
        bulge, y
    );
    return nextPos;
};
...
var LandingDamageHazard = function(chance, minTickScale, maxTickScale, dmgRange) {
    EntityDamageHazard.call(this, "landing", chance, minTickScale, maxTickScale);
    this.landingDmgRange = dmgRange;
};
...
var Planet = function(sun, systemSize, position){
    ...
    this.drawLighting = function(){
        ...
        fill(0, 0, 0, 20);
        arc(0, 0, newR*2, newR*2, -89, 90);
        arc(0, 0, newR*1.88, newR*2, 91, 270);
        var total = 0;
        for (var i = 1.88; i >= 0.8; i-=0.2){
            arc(0, 0, newR*i, newR*2, 91, 270);
            arc(0, 0, newR*2, newR*2, -89, 90);
            
        }
        for (var i = 1.2; i >= 0.8; i-=0.2){
            arc(0, 0, newR*2, newR*2, -89, 90);
            arc(0, 0, newR*i, newR*2, 91, 270);
        }
    ...
    // chance, minDmgTickScale, maxDmgTickScale, dmgRange
    ...
    this.drawRays = function(){
        ...
        for (var deg = 0; deg < 360; deg += random(5, 10)){
            ...
            triangle(this.x, this.y, 
                this.x+ cos(deg-this.rays[0]) * rayLength,
                this.y+ sin(deg-this.rays[0]) * rayLength,
                this.x+ cos(deg+this.rays[0]) * rayLength,
                this.y+ sin(deg+this.rays[0]) * rayLength
            );
            triangle(this.x, this.y, 
                this.x+ cos(deg-this.rays[1])*rayLength*1.5,
                this.y+ sin(deg-this.rays[1])*rayLength*1.5,
                this.x+ cos(deg+this.rays[1])*rayLength*1.5,
                this.y+ sin(deg+this.rays[1])*rayLength*1.5
            );  
            triangle(this.x, this.y, 
                this.x+ cos(deg-this.rays[2])*rayLength*2,
                this.y+ sin(deg-this.rays[2])*rayLength*2,
                this.x+ cos(deg+this.rays[2])*rayLength*2,
                this.y+ sin(deg+this.rays[2])*rayLength*2
            );             
        }
...
var BlackHole = function(position){
    ...
    this.drawSpiral = function (x, y, w, h, lf){
        var c = PI * w;
        var s = 0;
        for (var t = 0; t <= 360; t += 360.0/c){
            s = lf(t, c);
            var a = Math.cos(t) * w/2 * s;
            var o = Math.sin(t) * h/2 * s;
            strokeWeight(random(this.spiralWeight));
            point(floor(x + a) + random(-1, 1), floor(y + o) + random(-1, 1));
...
BlackHole.prototype.draw = function() {
    var timeBasedSize = map(sessionDuration % 1000, 0, 999, this.visibleSize * 0.95, this.visibleSize);
    strokeWeight(this.spiralWeight);
    var length = this.spiralLength;
    var randomLength = 5;
    length -= (randomLength + timeBasedBounce(1000, randomLength));
    var lengthFactor =  length * 0.003;
    var lengthFunction = function (theta, circum){
        return circum / theta * lengthFactor;
    };
    var slices = 5;
    ...
    for (var i = 1; i <= slices; ++i) {
        ...
        var hue = lerpColor(this.startColor, this.endColor, i/slices);
        var newColor = lerpColor(hue, color(135, 135, 135, alpha), 0.75);
        stroke(newColor);
        var sliceBasedSize = map(i, 1, slices, timeBasedSize * 0.75, timeBasedSize);
        this.drawSpiral(this.x, this.y, sliceBasedSize, sliceBasedSize, lengthFunction);    
    }
    noStroke();
};
//==========================================================
// Asteroid constructor ////////////////////////////////////
var asteroidRandomGen = new Random(1);
...
Asteroid.prototype.draw = function() {
    ...
    if (this.visibleSize){ 
       // pushMatrix();
            //rotate(this.rotation);
            image(asteroidImg, -(0.5 * this.visibleSize), -(0.65 * this.visibleSize), 
...
var Station = function(position){
    ...
    for (var i = 0; i < 5; i++) {
        ...
        ellipse(size * random(-0.01, 0.01), size * random(0.43, 0.45), (2 - displayFactor) * size * random(0.03, 0.05), 
            (1 + displayFactor) * size * (0.275 + random(0.15, 0.2)));
        fill(252, 252, 194, random(5, 30));
        ellipse(size * random(-0.01, 0.01), size * random(0.43, 0.45), (2 - displayFactor) * size * random(0.05, 0.07), 
            (1 + displayFactor) * size * (0.275 + random(0.15, 0.2)));
        fill(252, 179, 133, random(10, 32));
        ellipse(size * random(-0.01, 0.01), size * random(0.43, 0.45), (2 - displayFactor) * size * random(0.1, 0.12), 
            (1 + displayFactor) * size * (0.275 + random(0.05, 0.2)));
        fill(252, 207, 138, random(10, 33));
        ellipse(size * random(-0.01, 0.01), size * random(0.43, 0.45), (2 - displayFactor) * size * random(0.12, 0.14), 
            (1 + displayFactor) * size * (0.275 + random(0.1, 0.2)));
        fill(252, 202, 109, random(5, 34));
        ellipse(size * random(-0.01, 0.01), size * random(0.43, 0.45), (2 - displayFactor) * size * random(0.13, 0.15), 
            (1 + displayFactor) * size * (0.275 + random(0.05, 0.2)));
    }   
    // back ring     
        noFill();
        ...
        stroke(168, 168, 168);    
        arc(0, 0, 1*size, 0.5*size, 180, 270);
        stroke(161, 161, 161);    
        arc(0, 0, 1*size, 0.5*size, 270, 360);
        // back cross paths
        ...
        quad(size*0.15, size*(0.35), -size*0.15, 
            size*(0.35),
            -size*0.175, size*(0.35 + 0.05), size*0.175, 
            size*(0.35 + 0.05));
        ...
        triangle(size*0.025, -size*0.2, -size*0.025, 
            -size*0.2, 0, -size*0.9);
        ...
        stroke(184, 184, 184);    
        arc(0, -size*0.6, size*0.07, size*0.035, -70, 250);
        arc(0, -size*0.65, size*0.06, size*0.03, -70, 250);
        arc(0, -size*0.7, size*0.05, size*0.025, -70, 250);
        // front cross paths
        strokeWeight(size*0.06);
        stroke(158, 158, 158);
        ...
        strokeWeight(size*0.14);
        stroke(173, 173, 173);
        arc(0, 0, 1*size, 0.5*size, 0, 90);
        stroke(179, 179, 179);
        arc(0, 0, 1*size, 0.5*size, 90, 180);
...
space.refresh = function(){
    ...
    if (this.solarSystem.entities.length){
        var nextSystemEntity = this.solarSystem.entities[0];
    ...
    if (this.station){
        ...
        var newPos = info.distTraveled + random(1000, 2000);
...
landed.events = {
    ...
    tools: function (item){
        ...
        events.schedule(this.lastEventTime + 3300, function() { notices.add("e", dialog[0], 6600); });
        events.schedule(this.lastEventTime + 6600, function() { notices.add("e", dialog[1], 6600); });
        events.schedule(this.lastEventTime + 8300, function() { notices.add("e", dialog[2], 6600); });
        events.schedule(this.lastEventTime + 9900, function() { notices.add("g", gainMsg, 6600); });
        
        this.lastEventTime += 9900;
    ...
    vandalism: function (){
        ...
        var lastTime = this.lastEventTime;
        events.schedule(lastTime + 1650, function() { notices.add("e", "A mean looking alien lumbers forward.", 6600); });
        var vandalismDialog = [
        ...
        events.schedule(lastTime + 3300, function() { notices.add("e", vandalismDialog[0], 6600); });
        events.schedule(lastTime + 5000, function() { notices.add("e", vandalismDialog[1], 6600); });
        
        this.lastEventTime += 5000;
    },
    hullUpgrade: function (){
        var newHullRating = random( ship.s.hull.rating + 0.005, landed.e.technology );
        ship.s.hull.rating = newHullRating;
        ...
        events.schedule(this.lastEventTime + 1600, function() { notices.add("e", hullUpgradeDialog[0], 6600); }); // +50 delay
        events.schedule(this.lastEventTime + 3300, function() { notices.add("e", hullUpgradeDialog[1], 6600); }); // +100
        events.schedule(this.lastEventTime + 5000, function() { notices.add("e", hullUpgradeDialog[2], 6600); }); // +150
        
        events.schedule(this.lastEventTime + 6600, function() { notices.add("g", 
            "Your hull " + sInfo.hull.ratingTerm + " is now at " + round(newHullRating * 100) + "%", 6600); }); //+ 200
        
        this.lastEventTime += 6600;
    }
};
landed.doEvents = function (){
    // second inhabited check. possible non inhabited events?
    if (this.e.inhabited){ 
        this.events.lastEventTime = sessionDuration;
        ...
        chance = (random() < 0.05);
        var techDifference = this.e.technology - ship.s.hull.rating;
...
landed.getResources = function(){
    ...
    var sSupplies = ship.s;
    var totalResources = 0;
    var maxResources = 0;
    for (var r in eResources){ 
        // resources:property have a 1:1 ratio ... for now.
        totalResources += eResources[r].amnt;
        if (eResources[r].amnt > maxResources){
            maxResources = eResources[r].amnt;
        }
    }
    ...
    for (rName in eResources){
        ...
        if (rp && rp.amnt > 0.01){ //watch this
            ...
            if (sp.amnt === sp.cap){
                ...
                newAmnt = rp.amnt;
                if (sp.amnt + newAmnt > sp.cap){
                    newAmnt = sp.cap - sp.amnt;
                }
            }
            ui.sliders.add(
                "R" + rName, toCamelCase(rName) + ": " + round(rp.amnt * 100) + " " + rInfo[rName].UNIT,
                0, rp.amnt, newAmnt, 290, 340 - (newResourceCount * 20), 100, 16, 6, 13
            );
...
landed.receiveResource = function(newResource, newResourceAmnt){
    // resource object
    var ro = ship.landing.entity.resources[newResource]; 
    var ri = rInfo[newResource]; // resource info
    var so = ship.s[ri.property]; // supply object
    var si = sInfo[ri.property]; // supply info
    
    var newSupplyRating = ( (so.amnt * so.rating) + (newResourceAmnt * ro.rating) ) /
                          (so.amnt + newResourceAmnt);
                          
    //sanity check 'cause we're using floats
    ro.amnt = lowerLimit(ro.amnt - newResourceAmnt, 0);
    
    if (newResourceAmnt > 0.01){ 
        // we should really return partial percents ??
        // if no partial % check for 0.01 earlier I think.
        notices.add( "g", "You receive " + round(newResourceAmnt * 100) + " " + ri.UNIT + 
            " of " + round(ro.rating * 100) + "% " + si.RTNG + " " + newResource + ".", 13200);
        //color(147, 153, 207) 
        notices.add( "g", "Your " + ri.property + " " + si.ratingTerm + " is now at " + 
            round(newSupplyRating * 100) + "%", 16500);
    }
    
    // done mixing qualities, lets discard the overflow.
    if (so.amnt + newResourceAmnt > so.cap){
        newResourceAmnt = so.cap - so.amnt;
    } // do resource swap for better ql instead of mixing?
    so.rating = newSupplyRating;
...
var shipTools = {
    scanner: function (){
        ...
        if ( l.entity && l.entity.type === "planet" && l.progress > 1.0 - ship.tools.scanner.ql ){
            ...
            var info = "|=[ SCANNING ] \n|" + 
                spacing.substring(0, scanFrame) + "[=]" + spacing.substring(0, 23 - scanFrame) + "\n";
            ...
            for (var r in resourceList){
                ...
                if (planet.resources[rName].amnt > 0.01){
...
var shipHUD = {
    gauges: function(){
        ui.draw.gauge(
            360, 10, 10, 50, "T", ship.throttle, ship.speed/ship.p.speedMax);
        ui.draw.gauge(
            380, 10, 10, 50, "S", ship.speed/ship.p.speedMax, ship.throttle);
        // scale 0.333 for bigger cap values?
        ui.draw.gauge(
            360, 90, 10, 50, "F", 
            ship.s.fuel.amnt/ship.s.fuel.cap, 
            ship.s.fuel.amnt/ship.s.fuel.cap, 
            0.333
        );
        ...
        ui.draw.gauge(
            380, 90, 10, 50, "H", 
            ship.s.hull.amnt/ship.s.hull.cap, 
            ship.s.hull.amnt/ship.s.hull.cap, 
            warningThreshold
        );
        /*ui.draw.gauge(360, 170, 10, 50, "E", 
                    ship.s.energy.amnt, ship.s.energy.amnt);
...
state.list.selection.draw = function() {
    ...
    textFont(a, 12);
    // threshold
    fill(255, 255, 255, 100 * (sessionDuration / 5000) * ((sessionDuration > 3100) && (sessionDuration < 8300)) );
    text("and time", 0, 0);
    popMatrix();
    textAlign(LEFT, LEFT);
...
state.list.selection.mouseClicked = function() {
    if (mouseButton !== LEFT) {
        return;
    }
    //this.buttons.display();
    ...
    if (gameMode){
        resetEnvironment();
        //ship.throttle = 0.8/ship.p.speedMax; 
        ...
        if (gameMode === "freeFly"){
            ship.s.fuel.rating = 99999;
            ship.s.hull.rating = 99999;
            ship.tools = {
                scanner: { ql: 1 },
                radar: { ql: 1 }
            };
        } else if (gameMode === "journey"){
            ship.s.fuel.rating = 0.6;
            ship.s.hull.rating = 0.6;
        } else if (gameMode === "challenge"){
            ship.s.fuel.rating = 0.3;
            ship.s.hull.rating = 0.3;
            solarSystemFreq = 0.3;
        }
        // begin event registration //////////////////////////////////////////////////////////////////
        // condition, action, repeatTime (ms delay / optional)
        events.schedule(6500,  function() { notices.add("s", "Welcome to space.", 6700);} );
        events.schedule(13000, function() { notices.add("s", "Enjoy your journey.", 3400);} );
        events.schedule(20000, function() { notices.add("s", "It's a big place out there.", 3300);} );
        events.schedule(26500, function() { notices.add("d", "'I wonder how far we'll make it.'", 6600);} );
        ...
        events.register(function() { return (ship.s.fuel.amnt < 0.3 && ship.s.fuel.amnt > 0.3 - ship.s.fuelTick);},
                        function() { notices.add("w", "'Check your fuel, Captain.'", 16500);},
                        10000
        );
        events.register(function() { return (ship.s.fuel.amnt < 0.2 && ship.s.fuel.amnt > 0.2 - ship.s.fuelTick);},
                        function() { notices.add("w", "'We should refuel at a nearby planet.'", 16500);},
                        10000
        );
        events.register(function() { return (ship.s.hull.amnt < 0.3 && ship.s.hull.amnt > 0.3 - ship.dmgTick);},
                        function() { notices.add("w", "'Captain, We're taking damage.", 16500);},
                        10000
        );
        events.register(function() { return (ship.s.hull.amnt < 0.2 && ship.s.hull.amnt > 0.2 - ship.dmgTick); },
...
state.list.inSpace.draw = function() {
    ...
    // start key handling // this is handled here since key press events are erratic at the start of a key press
    if (keyIsPressed) {
...
state.define("landed", function() {
    ...
    if (landed.e.surfaceDmgTick){
        events.schedule(sessionDuration + 1600, function() {notices.add("w", "Something doesn't feel right...", 6600);});
    }
    else {
...
state.list.visiting.mouseClicked = function() {
    ...
    if (ui.buttons.list.back && ui.buttons.list.back.isPressed() ){
        ...
        if(landed.events.lastEventTime > sessionDuration){ 
            ...
            sFocus = ship.landing.entryPoint;
        ...
        for (var i = 0; i < landed.newResources.length;
            i++
        ){
...
state.define("gameOver", function() {
    ...
    ui.dialog.open(
        ...
        function(){ resetGame(); preserveGameMode();}, 
        function(){ resetGame();},
        "Restart", "Menu"
    );
    ui.dialog.display();
...
var keyReleased = function(){
    ...
    /*if (!keyIsPressed){
        keys = {}; // clear keys for efficiency // necessary?
    }*/
};
...
if (!DEBUG){
    try {
        ...
        if( ref.indexOf("5922406678921216") === -1 &&
            ...
            var msg = "T"+"h"+"e"+" "+"O"+"r"+"i"+"g"+"i"+"n"+"a"+"l"+" "+"V"+"e"+"r"+"s"+"i"+"o"+"n"+" "+"o"+"f"+" "+"t"+"h"+"i"+"s"+" "+"p"+"r"+"o"+"g"+"r"+"a"+"m"+" "+"i"+"s"+" "+"l"+"o"+"c"+"a"+"t"+"e"+"d"+" "+"a"+"t"+":"+" "+"h"+"t"+"t"+"p"+"s"+":"+"/"+"/"+"w"+"w"+"w"+"."+"k"+"h"+"a"+"n"+"a"+"c"+"a"+"d"+"e"+"m"+"y"+"."+"o"+"r"+"g"+"/"+"c"+"s"+"/"+"s"+"p"+"a"+"c"+"e"+"-"+"j"+"o"+"u"+"r"+"n"+"e"+"y"+"-"+"c"+"u"+"r"+"r"+"e"+"n"+"t"+"-"+"r"+"e"+"l"+"e"+"a"+"s"+"e"+"/"+"5"+"9"+"2"+"2"+"4"+"0"+"6"+"6"+"7"+"8"+"9"+"2"+"1"+"2"+"1"+"6";
            println(msg);
        }
    } catch(e){ println(e); }
}
